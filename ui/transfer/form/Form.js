...
import {  transferInitialState } from '@/utils/IntialState'
...
import { transferRequiredDbFields } from '@/utils/requireddbFields'
const Form = ({id, company, entity_code, fetchedData}) => {
    const router = useRouter();
    const endpoint = 'transfer';
    console.log("fetchedData", fetchedData);
    // Form data and state
    const [data, setData] = useState(fetchedData?fetchedData:{...transferInitialState, company: company, entity_code: entity_code});
    ...
    const handleSubmit = async () => {
        if (!validateForm(data, transferRequiredDbFields)) {
            setErrors({
                ...errors,
        ...
        try {
            ...
            toast.success(id === 'create' ? 'Transfer created successfully!' : 'Transfer updated successfully!');
        ...
        <div className={styles.form}>
            <div className={styles.form_header}>
                <h1 className={styles.form_title}>
                    {isCreateMode ? 'Create New Transfer' : 'Edit Transfer'}
                </h1>
            </div>
