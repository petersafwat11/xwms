...
const getIcon = (title) => {
  switch(title) {
    case 'Dashboard': return <FaHome className={classes.icon} />;
    case 'Scanning': return <FaQrcode className={classes.icon} />;
    case 'Inventory': return <FaBox className={classes.icon} />;
    case 'Reports': return <FaClipboardList className={classes.icon} />;
    case 'Settings': return <FaCog className={classes.icon} />;
    case 'Config': return <FaSliders className={classes.icon} />;
    case 'Billing': return <FaCreditCard className={classes.icon} />;
    default: return null;
  }
}
...
const Navbar = () => {
  ...
  return (
    <div className={`${classes.navbar} ${navbar.expanded ? classes.expanded : classes.collapsed}`}>
          ...
          <div key={item.title} className={classes.nav_item}>
            <div className={classes.nav_item_header}>
              {item.path ? (
                ...
                >
                  {getIcon(item.title)}
                  {navbar.expanded && <span className={classes.nav_title}>{item.title}</span>}
                </Link>
              ) : (
                <div 
                  className={classes.nav_link} 
                ...
                >
                  ...
                  {navbar.expanded && (
                    <>
                      ...
                      <FaChevronRight className={`${classes.arrow} ${item.active ? classes.arrow_active : ''}`} />
                    </>
                  )}
                </div>
              )}
