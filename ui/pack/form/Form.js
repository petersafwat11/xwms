...
import {  packInitialState } from '@/utils/IntialState'
...
import { packRequiredDbFields } from '@/utils/requireddbFields'
const Form = ({id, company, entity_code, fetchedData}) => {
    const router = useRouter();
    const endpoint = 'pack';
    console.log("fetchedData", fetchedData);
    // Form data and state
    const [data, setData] = useState(fetchedData?fetchedData:{...packInitialState, company: company, entity_code: entity_code});
    ...
    const handleSubmit = async () => {
        if (!validateForm(data, packRequiredDbFields)) {
            setErrors({
                ...errors,
        ...
        try {
            ...
            toast.success(id === 'create' ? 'Pack created successfully!' : 'Pack updated successfully!');
        ...
        <div className={styles.form}>
            <div className={styles.form_header}>
                <h1 className={styles.form_title}>
                    {isCreateMode ? 'Create New Pack' : 'Edit Pack'}
                </h1>
            </div>
