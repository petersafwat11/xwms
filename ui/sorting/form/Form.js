...
import {  sortingInitialState } from '@/utils/IntialState'
...
import { sortingRequiredDbFields } from '@/utils/requireddbFields'
const Form = ({id, company, entity_code, fetchedData}) => {
    const router = useRouter();
    const endpoint = 'sorting';
    console.log("fetchedData", fetchedData);
    // Form data and state
    const [data, setData] = useState(fetchedData?fetchedData:{...sortingInitialState, company: company, entity_code: entity_code});
    ...
    const handleSubmit = async () => {
        if (!validateForm(data,sortingRequiredDbFields)) {
            setErrors({
                ...errors,
        ...
        try {
            ...
            toast.success(id === 'create' ? 'Sorting created successfully!' : 'Sorting updated successfully!');
    ...
    const handleDelete = async () => {
        if (window.confirm('Are you sure you want to delete this record?')) {
            ...
            try {
                ...
                toast.success('Sorting deleted successfully!');
                router.push(`/${endpoint}`);
            } catch (error) {
                toast.error(error.message || 'An error occurred');
                setErrors({
        ...
        <div className={styles.form}>
            <div className={styles.form_header}>
                <h1 className={styles.form_title}>
                    {isCreateMode ? 'Create New Sorting' : 'Edit Sorting'}
                </h1>
            </div>
