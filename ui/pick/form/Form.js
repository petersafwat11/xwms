...
import {  pickInitialState } from '@/utils/IntialState'
...
import { pickRequiredDbFields } from '@/utils/requireddbFields'
const Form = ({id, company, entity_code, fetchedData}) => {
    const router = useRouter();
    const endpoint = 'pick';
    console.log("fetchedData", fetchedData);
    // Form data and state
    const [data, setData] = useState(fetchedData?fetchedData:{...pickInitialState, company: company, entity_code: entity_code});
    ...
    const handleSubmit = async () => {
        if (!validateForm(data, pickRequiredDbFields)) {
            setErrors({
                ...errors,
        ...
        try {
            ...
            toast.success(id === 'create' ? 'Pick created successfully!' : 'Pick updated successfully!');
        ...
        <div className={styles.form}>
            <div className={styles.form_header}>
                <h1 className={styles.form_title}>
                    {isCreateMode ? 'Create New Pick' : 'Edit Pick'}
                </h1>
            </div>
