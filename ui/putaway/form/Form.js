...
import {  putawayInitialState } from '@/utils/IntialState'
...
import { putawayRequiredDbFields } from '@/utils/requireddbFields'
const Form = ({id, company, entity_code, fetchedData}) => {
    const router = useRouter();
    const endpoint = 'putaway';
    console.log("fetchedData", fetchedData);
    // Form data and state
    const [data, setData] = useState(fetchedData?fetchedData:{...putawayInitialState, company: company, entity_code: entity_code});
    ...
    const handleSubmit = async () => {
        if (!validateForm(data,putawayRequiredDbFields)) {
            setErrors({
                ...errors,
        ...
        try {
            ...
            toast.success(id === 'create' ? 'Putaway created successfully!' : 'Putaway updated successfully!');
    ...
    const handleDelete = async () => {
        if (window.confirm('Are you sure you want to delete this record?')) {
            ...
            try {
                ...
                toast.success('Putaway deleted successfully!');
                router.push(`/${endpoint}`);
            } catch (error) {
                toast.error(error.message || 'An error occurred');
                setErrors({
        ...
        <div className={styles.form}>
            <div className={styles.form_header}>
                <h1 className={styles.form_title}>
                    {isCreateMode ? 'Create New Putaway' : 'Edit Putaway'}
                </h1>
            </div>
